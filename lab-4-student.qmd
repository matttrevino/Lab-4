...
buoy <- read.table("buoy_2024.txt", na = c("99", "99.0", "99.00", "999", "999.0", "999.00"), header = TRUE)
write.csv(buoy, "buoy.csv")

buoy <- buoy %>%
  clean_names()

# Mutate only numeric columns
buoy_final <- buoy %>%
  mutate(across(.cols = c("mm", "atmp"), ~parse_double(as.character(.x))))

write.csv(buoy_final, "buoy_final.csv")
buoy_final

#sorry I was following the github page and did it all in one chunk instead of below....
```
...
#makes the x axis have proper discrete values (why was this so hard to figure out)
library(ggplot2)
#used Chatgpt to figure out how to make "mm" display as a numeric variable for the sake of the graph because nothing else I tried was working :( -> made x as.numeric(mm). 
